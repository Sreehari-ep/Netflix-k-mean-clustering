...
if find_btn:
    with st.spinner("Finding the perfect content for you..."):
        ...
        # If no results, show all with high ratings
            ...
            with cols[idx % 3]:
                with st.container():
                    ...
                    <div style="background: white; border-radius: 12px; padding: 1rem; margin-bottom: 1rem; 
                                box-shadow: 0 2px 8px rgba(0,0,0,0.08); border-left: 4px solid {rating_color};">
                        <h4 style="margin: 0; font-size: 1rem;">{row['title']}</h4>
                        <div style="display: flex; gap: 0.5rem; margin-top: 0.5rem; flex-wrap: wrap;">
                            <span style="background: #f0f0f0; padding: 0.2rem 0.6rem; border-radius: 10px; font-size: 0.8rem;">
                                ⭐ {row['rating']:.1f}
                            </span>
                            <span style="background: #f0f0f0; padding: 0.2rem 0.6rem; border-radius: 10px; font-size: 0.8rem;">
...
# Show some initial stats when no prediction
    ...
    for idx, (_, row) in enumerate(popular_df.iterrows()):
        if idx < 8:
            with cols[idx % 4]:
                st.markdown(f"""
                <div style="background: #f8f9fa; border-radius: 10px; padding: 0.8rem; margin-bottom: 0.5rem;
                            border-left: 3px solid #E50914;">
                    <div style="font-weight: 600; font-size: 0.9rem;">{row['title']}</div>
                    <div style="font-size: 0.8rem; color: #666;">⭐ {row['rating']:.1f} · {row['type']}</div>
                </div>
                """, unsafe_allow_html=True)
